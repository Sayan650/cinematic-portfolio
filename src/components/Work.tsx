...
const projects = [
  ...
  },
  {
    ...
    description:
      "An AI-powered AIOps and incident management platform that correlates application metrics, centralized logs, and infrastructure signals to automate incident investigation, generate root-cause analysis, and provide intelligent remediation recommendations.",
    tech: [
      "Python",
      "FastAPI",
      "Docker",
    ...
    image: "/robot.png",
    github: "https://github.com/Sayan650/AI-SRE",
    live: "#",
    featured: true,
    layout: "wide",
  },
    ...
    >
      <div
        className={`relative overflow-hidden group rounded-sm ${
          project.layout === "wide"
            ? "w-full md:w-3/4 aspect-video"
            : project.layout === "portrait"
              ? "w-full md:w-1/2 aspect-[3/4]"
      ...
      >
        ...
        <p
          className={`mt-4 text-sm text-muted-gray max-w-md whitespace-pre-wrap ${isEven ? "" : "ml-auto"}`}
        >
          {project.description}
        </p>
        <p
          className={`mt-2 text-[10px] uppercase tracking-widest text-muted-lime font-medium ${isEven ? "" : "ml-auto"}`}
        >
          {project.tech.join(" • ")}
        </p>
        <div className={`mt-6 flex gap-6 ${isEven ? "" : "justify-end"}`}>
          {project.live && project.live !== "#" ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-[10px] uppercase tracking-[0.3em] font-medium border-b border-kimono-white/20 pb-1 hover:border-muted-lime hover:text-muted-lime transition-all duration-500"
            >
              Live Demo
            </a>
          ) : null}
  ...
  return (
    <section id="work" className="relative py-32 px-8 md:px-20 overflow-hidden">
      ...
      <div className="relative z-10 mb-32">
        <h2 className="text-xs uppercase tracking-[0.6em] text-muted-gray mb-8">
